...
for i in `seq 0 $len` ; do 
	...
	echo "== cbar $name"
	cbar_pred=$outdir/$name\_cbar_prediction.txt
	~/PlasFlow/filter_sequences_by_length.pl -input $contigs -output $tmp_dir/tmp.fasta -thres 1000 
	~/cBar.1.2/cBar.pl tmp.fasta $cbar_pred 
	#python3 bin/cbar_fasta.py $cbar_pred $contigs $name $outdir  
done 	
